...
class UserReminders(commands.Cog):
    ...
    async def add_personal_reminder(self, ctx, title: str, due_date: str, message: str = ""):
        """
        Add a new personal reminder.
        """
        try:
            duedate = parser.parse(due_date)
            db.query("INSERT INTO personal_reminders (user_id, title, due_date, message) VALUES (%s, %s, %s, %s)",
    ...
    async def delete_personal_reminder(self, ctx, reminder_id: int):
        ...
        """
        result = db.query("DELETE FROM personal_reminders WHERE id = %s AND user_id = %s RETURNING *",
    ...
    async def list_personal_reminders(self, ctx):
        ...
        """
        reminders = db.query("SELECT id, title, due_date, message FROM personal_reminders WHERE user_id = %s",
                             (ctx.author.id,))
        if reminders:
            embed = discord.Embed(title="Your Personal Reminders", color=0x00ff00)
            for reminder in reminders:
                embed.add_field(name=f"ID: {reminder[0]}, Title: {reminder[1]}", 
    ...
    async def update_personal_reminder(self, ctx, reminder_id: int, new_due_date: str, new_message: str = ""):
        """
        Update a specific personal reminder.
        """
        try:
            new_duedate = parser.parse(new_due_date)
            db.query("UPDATE personal_reminders SET due_date = %s, message = %s WHERE id = %s AND user_id = %s",
    ...
    async def check_reminders(self):
        ...
        reminders = db.query("SELECT user_id, title, message FROM personal_reminders WHERE due_date <= %s AND notified = FALSE",
                             (now,))
        for reminder in reminders:
            user = self.bot.get_user(reminder[0])
            if user:
                await user.send(f"**Reminder:** {reminder[1]} - {reminder[2]}")
                db.query("UPDATE personal_reminders SET notified = TRUE WHERE title = %s AND user_id = %s", 
