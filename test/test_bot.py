...
async def test_qanda(bot):
    ...
    # Tests that the bot sent the appropriate message (answer)
    assert dpytest.verify().message().contains().content(
        'Please send answers to the #q-and-a channel.')
    
    # ask a question and remove it from the queue
    await dpytest.message("$ask \"What class is this?\" anonymous", channel=channel)
    assert dpytest.verify().message().contains().content(
        'Q3: What class is this? by anonymous')
    
    # lmid = channel.last_message_id
    # hold on to the ID of the question generated by the bot
    # lmid = await channel.fetch_message(channel.last_message_id)
    # print(lmid.content)
    # will print:
    # Q3: What class is this? by anonymous

    # Test answering a question anonymously
    await dpytest.message("$answer 3 \"Test\" anonymous", channel=channel)
    #assert dpytest.verify().message().contains().content(
    #    'Q3: What class is this? by anonymous\nanonymous (Instructor) Ans: Test\n')

    #assert lmid.content == 'Q3: What class is this? by anonymous\nanonymous (Instructor) Ans: Test\n'

    # Test answering a question with a name
    await dpytest.message("$answer 3 \"Test\" anonymous", channel=channel)
    #assert dpytest.verify().message().contains().content(
    #    'Q3: What class is this? by anonymous\n
    #    'anonymous (Instructor) Ans: Test\n'
    #    +user.name +' (Instructor) Ans: Test\n'
    # )


    # Tests that the bot sends the appropriate message for unknown anonymous input (answer)
